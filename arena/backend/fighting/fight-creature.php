    ...
    if ($adventure == 1){
		...
		function whereToHit()
                ...
                $players[$player]['battleSurrender'] = round($players[$player]['vitality'] * $yourSurrender); 
                    ...
                    $rangedAttackMod = (($players[$player]['dexterity'] * $row['dexAttackMod'])*2) + 1;
					...
					#HP Regeneration
					if ($players[$player]['race'] == "Dryad"){
						switch($players[$player]['level']){
							case 1:
							case 2:
							case 3:
							case 4:
							case 5:
								$players[$player]['minRegenHp'] = 1;
								$players[$player]['maxRegenHp'] = 1;
								break;
							case 6:
							case 7:
							case 8:
							...
							case 14:
							case 15:
								$players[$player]['minRegenHp'] = 2;
								$players[$player]['maxRegenHp'] = 3;
								break;
							default:
								$players[$player]['minRegenHp'] = 2;
								$players[$player]['maxRegenHp'] = 4;
								break;
						}
					}
...
function regenerateHP($player){
	global $players,$yourReport;
	
	$regenRate = mt_rand($players[$player]['minRegenHp'], $players[$player]['maxRegenHp']);
	
	if($players[$player]['hp'] === $players[$player]['vitality']){
		#Nothing happens, full hp
	}
	else{
		$diffBetweenMax = $players[$player]['vitality'] - $players[$player]['hp'];
		...
		$yourReport .= "<span style='color:green'> " . $players[$player]['name'] . " regenerated <strong>" . $regenRate . " hp</strong></span><br>";
	}
}
    							...
    							if ($foul !== "true"){  									
    									...
    									if (mt_rand(1,100) <= $players[0]['parry'] && $part != "shield"){
    										...
    										if($part == "shield"){
												...
												if (mt_rand(1,100) <= 30){
													#critical block
													$yourReport .= $hitText . "." . "<strong> " . $players[0]['name'] . " critically blocks the attack with " . $players[0]['hisher'] . " " . $players[0]['left_hand']['name'] . " completely negating the " . round($dmg) . " damage!</strong><br>";
													$dmg = 0;
												}
												else{
	    										...
	    										$dmg = round((rand($opponentMinDmg,$opponentMaxDmg) * $opponentAttackMod));
	    											...
	    											if($players[0][$part]['damage_reduction'] == 0){
	    												...
	    												if (($dmg-$players[0][$part]['damage_reduction']) <= 0){
	    													...
	    													$damageAbsorb = $players[0][$part]['damage_reduction'];
															$damageAbsorbPercent = (($damageAbsorb * $armourPercent) / 100);
															#echo $damageAbsorbPercent;
															if(round($dmg * $damageAbsorbPercent) > $damageAbsorb){
																$absorbedDamage = round($dmg * $damageAbsorbPercent);
	    													...
	    													$damageText = "<br>" . $players[0]['name'] . " is protected by " . $players[0]['hisher'] . " " . $players[0][$part]['name'] . " which soaks up " . $absorbedDamage . " damage from " . $players[0]['hisher'] . " " . "opponents attack! ";
	    													$damageValueText = "The hit dealt <strong>" . $dmg . " damage!</strong>" . "<br><br>";
	    												}
	    											}
...
while($players[0]['hp'] > $players[0]['battleSurrender'] && $opponentHp > 0 && $i < 51){
usleep(60000);
$firstMove = 0;
    ...
    elseif($players[0]['hp'] > $players[$player]['battleSurrender']){
		...
		if($guildId != 0){
			$sql = "UPDATE guilds SET experience=experience+'$opponentXp' WHERE id='$guildId'";
			mysqli_query($conn,$sql);
		}
